 ...
 * <p>
 * 未打刻者の一覧を所属部門ごとにグルーピングし、出勤予定時刻および当月の累積未打刻回数を盛り込んだ 管理者向けトークルームで視認しやすいフォーマット済みのアラートメッセージ文字列を構築します。
 * </p>
 */
@Service
public class UnstampedAlertMessageFormatter {
   ...
   */
  public String format(LocalDate targetDate,
      List<UnstampedAlertEvent.UnstampedEmployee> employees) {
    ...
    grouped.forEach((deptName, list) -> {
      ...
      for (UnstampedAlertEvent.UnstampedEmployee emp : list) {
        ...
        sb.append(
            String.format("  ・%s （予定: %s〜 / 今月%d回目）%n", emp.fullName(), timeStr, count));
      }
      sb.append("\n");
    });
