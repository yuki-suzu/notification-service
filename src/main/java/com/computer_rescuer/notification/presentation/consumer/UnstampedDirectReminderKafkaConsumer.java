 ...
 * <p>
 * 受信したイベントを検証後、{@link SendUnstampedDirectReminderUseCase} へディスパッチします。<br>
 * 再試行上限超過時は DLT ハンドラーを通じてシステム管理者に障害を通知します。
 * </p>
 */
@Slf4j
@Component
...
public class UnstampedDirectReminderKafkaConsumer {
    ...
    /**
     * 未打刻者ダイレクト通知トピックからメッセージを購読（Consume）します。
     *
    ...
    public void consume(
        ...
        log.info("📥 [Kafka受信: 未打刻DM] Topic: {}, Partition: {}, Offset: {}, 社員番号: {}, Email: {}",
                topic, partition, offset, event.employeeNumber(), event.email());

        sendUnstampedDirectReminderUseCase.execute(event);
    }

    /**
     * 未打刻個別DMのリトライ上限到達時における DLT 退避ハンドラー。
     *
